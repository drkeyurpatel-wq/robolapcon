...
import { CheckCircle2, AlertCircle, RotateCcw, ScanLine, Search } from 'lucide-react';
...
export default function CounterScan({ counter }: { counter: Counter }) {
  ...
  const title = counter === 'food' ? '🍽 Food Counter' : '🎁 Gift Counter';

  // Record the scan; for the gift counter also queue the certificate.
  const record = useCallback(async (id: string) => {
    ...
    if (res?.success) {
      ...
      setResult({ type: 'success', name: res.delegate_name, text: `${counter === 'food' ? '🍽' : '🎁'} ${res.delegate_name} — ${counter} collected${extra}` });
      setScanCount((c) => c + 1);
